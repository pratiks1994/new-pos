...
const getLiveOrders = () => {
	try {
		...
		const prepareItem = db2.prepare(
			"SELECT id,item_id,item_name,price,final_price, item_discount,quantity,variation_name,variation_id,tax_id,item_addon_items,description FROM order_items WHERE order_id = ?"
		);

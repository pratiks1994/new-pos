...
const createKot = async (order, db, userId) => {
      ...
      const KOTId = await dbRun(
            ...
            [restaurantId, tokenNo, orderType, userId, customerName, customerContact, customerAdd, customerLocality, tableNumber, tableNumber, 1, "active"]
      );

      orderCart.forEach(async (item) => {
            const { itemQty, itemId, itemName, variation_id, variantName, toppings } = item;

            const KOTItemId = await dbRun(db, "INSERT INTO KOT_items (KOT_id,item_id,item_name,quantity,variation_name,variation_id,description) VALUES (?,?,?,?,?,?,?)", [
                  KOTId,
                  itemId,
                  itemName,
                  itemQty,

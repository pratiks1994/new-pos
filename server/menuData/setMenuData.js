...
const setMenuData = async (token) => {
	...
	var options = {
		method: "POST",
		url: "https://martinozpizza.emergingcoders.com/api/pos/v1/get-menu",
		headers: {
			Accept: "*/*",
	...
	try {
		const categorystmt = db3.prepare(
		...
		const addongroupstmt = db3.prepare("INSERT INTO addongroups (id,restaurant_id,main_addongroup_id,name,display_name,priority,status,pp_id) VALUES (?,?,?,?,?,?,?,?)");

		const addonItemstmt = db3.prepare("INSERT INTO addongroupitems (id,attribute,addongroup_id,name,display_name,price,status,priority,pp_id) VALUES (?,?,?,?,?,?,?,?,?)");

		const itemstmt = db3.prepare(
		...
		const itemVariationstmt = db3.prepare("INSERT INTO item_variation (id,item_id,variation_id,price,restaurant_price_id) VALUES (?,?,?,?,?)");

		const itemRestaurantPricesstmt = db3.prepare("INSERT INTO item_restaurant_prices (item_id,restaurant_price_id,price,status) VALUES (?,?,?,?)");

		const addonItemVariationstmt = db3.prepare("INSERT INTO addongroup_item_variation (id,addongroup_id,item_variation_id) VALUES (?,?,?) ");

		const taxstmt = db3.prepare("INSERT INTO taxes (id,restaurant_id,main_tax_id,name,tax,order_types,status,priority,pp_id) VALUES (?,?,?,?,?,?,?,?,?)");

		const dineInTablestmt = db3.prepare("INSERT INTO dine_in_tables (id,restaurant_id,area_id,uid,sid,table_no,qr_code,platform) VALUES (?,?,?,?,?,?,?,?)");

		const groupstmt = db3.prepare("INSERT INTO groups (id,name,permissions) VALUES (?,?,?)");

		const userstmt = db3.prepare("INSERT INTO users (id,name,number,otp,email,pg_cust_id,email_verified_at,activated) VALUES (?,?,?,?,?,?,?,?)");

		const userGroupstmt = db3.prepare("INSERT INTO users_groups (user_id,group_id) VALUES (?,?) ");

		const pricesstms = db3.prepare("INSERT INTO prices (id,name,type,status,created_at,updated_at) VALUES(?,?,?,?,?,?)");

		const restaurantPricesstmt = db3.prepare("INSERT INTO restaurant_prices (id,price_id,restaurant_id,display_name,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?)");

		const orderTypestmt = db3.prepare("INSERT INTO order_types (id,name,status) VALUES(?,?,?)");

		const orderstmt = db3.prepare(
		...
		const orderItemstmt = db3.prepare(
		...
		const orderItemTaxesstmt = db3.prepare("INSERT INTO order_item_taxes (id,order_item_id,tax_id,tax,tax_amount,created_at,updated_at) VALUES (?,?,?,?,?,?,?)");

		const brandsstmt = db3.prepare(`
		...
		const attributesstmt = db3.prepare(`INSERT INTO attributes (id, name, status, pp_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`);

   ...
   return (
      <div className={styles.finalOrderItem}>
         ...
         <div className={styles.name}>
            <span className={styles.remove} onClick = {()=>{remove(currentOrderItemId)}}> x </span>{` ${itemName} `}</div>
         <div className={styles.qty}>
            <button onClick={()=>removeQty(currentOrderItemId)} className={styles.btn}>-</button> 
            {itemQty} 
            <button className={styles.btn} onClick={()=>addQty(currentOrderItemId)}>+</button> 
         </div>

...
function OrderType() {
	...
	const handleOrderType = (orderType) => {
		if (orderType === "delivery" || orderType === "pick_up") {
			dispatch(setActive({ key: "restaurantPriceId", name: null }));
	...
	return (
		<div className="flex-shrink-0">
			<div className={`d-flex ${styles.orderTypeBtn}`}>
				<button
					className={`flex-grow-1 ${orderType === "dine_in" ? "bg-danger" : "none"}`}
					onClick={() => handleOrderType("dine_in")}>
					Dine In
				</button>

				<button
					className={`flex-grow-1 ${orderType === "delivery" ? "bg-danger" : "none"}`}
					onClick={() => handleOrderType("delivery")}>
					Delivery
				</button>
				<button
					className={`flex-grow-1 ${orderType === "pick_up" ? "bg-danger" : "none"}`}
					onClick={() => handleOrderType("pick_up")}>
					Pick up
				</button>
			</div>

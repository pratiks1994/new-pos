...
function AssignBill() {
		...
		onSuccess: (data) => {
			...
			setPrinterBillOrderType((prev) => {
				return prev.map((orderType) => {
					...
					const isChecked = selectedPrinter.bill_print_ordertypes.includes(id.toString()) || false;
					const copyCount = selectedPrinter.bill_print_copy_count.split(",")[id - 1] || 1;
	...
	const handleSave = async () => {
		...
		const updateData = { assignType, printerId, billPrintCopyCount, billPrintOrderTypes };
	...
	return (
		<motion.div
			...
			<header>
				<div className={styles.headerText}> Assign Printer to Bill - {printerName}</div>
